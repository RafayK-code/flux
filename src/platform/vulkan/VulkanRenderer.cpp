...
namespace flux
{
        ...
        : currentFrameInFlight_(0), graphicsContext_(graphicsContext)
    {
        ...
        ShaderUniformLayout layout = {
            { "texSampler", 0, UniformType::Sampler, ShaderStage::Fragment },
        };
        Ref<Shader> shader = Shader::Create("assets/shaders/texture.vert.spv", "assets/shaders/texture.frag.spv", layout);
    ...
    {
        swapchainRenderPass_->GetShader()->PushSampler(finalImage, 0, currentFrameInFlight_);
    ...
    void VulkanRenderer::Draw(const Ref<RenderPass>& renderPass, const Ref<VertexArray>& vertexArray)
    {
        drawCommands_.emplace_back([=](VkCommandBuffer cmd) {
            ...
            vkCmdDrawIndexed(cmd, indexBuffer->Count(), 1, 0, 0, 0);
